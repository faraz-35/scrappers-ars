...
def run_scrapper():
    ...
    total_articles = pd.DataFrame(columns = ['Source', 'Link','DateOfExtraction','DateOfPublication','Title','Author','Article'])
    for i in range (len(unique_page_links1)):
        ...
        try:
            ...
            src = "Source:"+source+"\n"
            lnk = "Link:"+url+"\n" 
            doe = datetime.today().strftime('%Y-%m-%d')
            dop = article.publish_date.strftime('%Y-%m-%d') if(article.publish_date) else '2023-03-12'
            ttl = article.title
            auth = ""
           ...
           print("Error")
        # new_row=pandas.Dataframe({'Source':source, 'Link':url,'DateOfExtraction':doe,'DateOfPublication':dop,'Title':ttl,'Author':auth,'Article':article.text})
        new_row = {'Source':source, 'Link':url,'DateOfExtraction':doe,'DateOfPublication':dop,'Title':ttl,'Author':auth,'Article':article.text}
        # total_articles=total_articles.append(new_row)
        total_articles = pd.concat([total_articles, pd.DataFrame(new_row, index=[0])], ignore_index=True)

        # temp =pd.Dataframe(new_row,columns = ['Source', 'Link','DateOfExtraction','DateOfPublication','Title','Author','Article'])
        # total_articles= pd.concat([temp,total_articles])
